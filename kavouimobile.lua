    ...
    frame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End or input.UserInputState == Enum.UserInputState.Touch then
                    dragging = false
                end
            end)
        end
...
function Kavo.CreateLib(kavName, themeList)
    ...
    local function updateSlider(positionX)
        local relativeX = math.clamp(positionX - sliderBtn.AbsolutePosition.X, 0, 149)
    ...
    sliderBtn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch and not focusing then
            game.TweenService:Create(val, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), { TextTransparency = 0 }):Play()
            updateSlider(input.Position.X)

            local touchMovedConn, touchEndedConn

            touchMovedConn = input.TouchMoved:Connect(function(moveInput)
                updateSlider(moveInput.Position.X)
            end)

            touchEndedConn = input.TouchEnded:Connect(function()
                game.TweenService:Create(val, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), { TextTransparency = 1 }):Play()
                if touchMovedConn then touchMovedConn:Disconnect() end
